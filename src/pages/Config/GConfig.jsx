  ...
  const fields = [
    {
      ...
      component: <Select defaultValue={currentEnv} placeholder="选择对应环境">
        {
          envList.map(v => <Option value={v.id} key={v.key}>{v.name}</Option>)
        }
      </Select>,
      type: 'select',
    },
        ...
        <Row gutter={[8, 8]}>
          ...
          <Col span={8}>
            <Input  addonBefore={<Select allowClear placeholder="选择对应环境" value={currentEnv} style={{width: 120}}
              ...
              {
                envList.map(v => <Option value={v.id.toString()} key={v.id}>{v.name}</Option>)
              }
            </Select>}
              placeholder='请输入key' value={name} onChange={e => {

...
import { Alert, Button, Card, Col, Form, Input, Modal, notification, Row, Select, TreeSelect } from 'antd';
...
const TestCaseRecorder = ({dispatch,project, testcase,recorder, loading}) => {
  ...
  return (
    <PageContainer breadcrumb={null}
                   title={<span className="ant-page-header-heading-title">用例录制
                     <CameraOne theme="outline" size="18" fill="#7ed321"  strokeLinecap="square"/> </span>}>
      <Card>
        ...
        <Row gutter={8} style={{marginTop: 12}}>
          <Col span={24}>
            <RequestInfoList rowSelection={rowSelection} rowKey="index" emptyText="点击录制按钮即可开始录制app/web的接口请求"
                             dataSource={recordLists} dispatch={dispatch} loading={loading.effects['recorder/queryRecordStatus']}/>
          </Col>
        </Row>
      </Card>
    </PageContainer>)
...
export default connect(({loading, recorder, project, testcase, global}) => ({global, recorder,project, testcase, loading}))(TestCaseRecorder);
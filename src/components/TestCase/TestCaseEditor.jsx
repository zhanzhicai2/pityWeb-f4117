  ...
  return (
    ...
    >
      <Card title={<span className={styles.caseTitle}>用例信息</span>}
        ...
        <Row gutter={[8, 8]}>
          {
            fields.CaseDetail.map(item => <Col span={item.span || 24} key={item.id}>
        ...
        <Row style={{marginTop:8}}>
          <Col span={24}>
            <TestCaseBottom case_id={caseId} body={body} bodyType={bodyType} setBody={setBody} headers={headers}
                            setHeaders={setHeaders} form={form} createMode={create} formData={formData}
                            setFormData={setFormData} setSuffix={setSuffix} setBodyType={setBodyType}
            />
          </Col>
        </Row>
      </Card>
